      ...
      <div class="orderInfo">
        ...
        <div class="wrap">
          ...
          <ul class="infoItem">
            ...
            <li v-if="orderData.integral&&(orderData.status==7||orderData.status==8)"> <span class="name" >获取积分：</span><span class="content">{{orderData.integral}}</span></li>
          </ul>
        </div>
      </div>
      <!--
  ...
  methods: {
    ...
    orderDetailRender () {
      ...
      http(orderDetails, [orderCode]).then((res) => {
        ...
        storage.setLocalStorage(invoiceAmount, res.data.body.invoiceAmount)
        let _this = this
