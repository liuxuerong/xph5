      ...
      <div id="goodsinfo">
        <div v-for="(item,index) in keys" :key="index">
          <div class="tabContent">
            {{item.name}}:
            <input type="button" class="sku" @click="tabInfoChange(index,cindex,citem.id,$event)" v-for="(citem,cindex) in item.value" :class="{notClick:citem.notClick,active:citem.isActiveC}" :attr_id="citem.id" :value="citem.id" :key="cindex"/>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    // 初始化得到结果集
    initSKU () {
      var i = this.getObjKeys(this.data)
      var j = this.getObjKeys(this.data)
