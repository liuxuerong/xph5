...
export default {
  ...
  data () {
    return {
      ...
      skuList: [{
        'color': '银色',
        'size': '13.3英寸',
        'config': 'Core i5/8G内存/256G闪存'
      },
      {
        'color': '银色',
        'size': '15.4英寸',
        'config': '512闪存/Multi-Touch Bar'
      },
      {
        'color': '深空灰色',
        'size': '13.3英寸',
        'config': '256闪存/Multi-Touch Bar'
      },
      {
        'color': '银色',
        'size': '13.3英寸',
  ...
  created () {
    ...
    this.addStateToType()
    // 监听color-change事件
    this.$root.$on('color-change', (target) => {
      this.colorChangeHandler(target)
    })

    // 监听size-change事件
    this.$root.$on('size-change', (target) => {
      this.sizeChangeHandler(target)
    })

    // 监听config-change事件
    this.$root.$on('config-change', (target) => {
    ...
    attrChangeHandler (attr, attrInfo) {
      ...
      if (attrState === 2) {
        ...
        if (count === 0) {
          this.resetTypeSelectedState()
          this.selectRightData(this.skuList, attr, attrInfo)
        } else { // 新选// 如果count != 0，则是已有属性被选择，则需要判断这次是新选还是更改
